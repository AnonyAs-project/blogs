...
const Navbar = () => {
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-3 flex items-center justify-between">
      ...
      {isNotificationsOpen && (
        <div className="absolute z-[500] mt-2 top-12 left-0 bg-gray-700 w-[100%] md:w-[35%] max-h-72 rounded-lg shadow-xl overflow-y-auto border border-gray-600">
          <div className="py-2">
            {notifications?.length > 0 ? (
              notifications.map((notification) => (
                <div
                  key={notification._id}
                  className={`relative p-4 border-b border-gray-600 rounded-lg transition-all ease-in-out duration-200 my-2 cursor-pointer ${notification.read ? 'bg-gray-600' : 'bg-[#0000001a]'}`}
              ...
              <div className="flex justify-center items-center p-6 bg-gray-600 rounded-lg shadow-md text-gray-400 text-lg font-medium">
                No Notifications
              </div>
            )}
          </div>
      ...
      <button
        ...
        className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded-md shadow-md transition"
      >
        Logout
      </button>
    </div>
